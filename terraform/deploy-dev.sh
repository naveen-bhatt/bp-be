...
# This script deploys the Development infrastructure using Terraform
...
echo -e "${BLUE}🚀 Development Environment Deployment${NC}"
echo "=========================================="
...
if ! aws ssm get-parameter --name "/dev/database_url" --region ap-south-1 &> /dev/null; then
    echo -e "${RED}❌ Parameter Store parameters not found. Please run the setup script first.${NC}"
    echo -e "${YELLOW}💡 Run: ./scripts/setup-parameter-store.sh dev${NC}"
    exit 1
fi
...
cd "$SCRIPT_DIR"
if ! ./validate-infrastructure.sh dev; then
    echo -e "${RED}❌ Infrastructure validation failed${NC}"
    exit 1
fi

echo -e "${GREEN}🎉 Development Environment deployment completed successfully!${NC}"
echo ""
echo -e "${BLUE}📋 Deployment Summary:${NC}"
echo -e "${BLUE}   Environment: Development${NC}"
echo -e "${BLUE}   ECS Cluster: dev-bluepansy-cluster${NC}"
echo -e "${BLUE}   ECS Service: dev-bluepansy-service${NC}"
echo -e "${BLUE}   ALB: dev-bluepansy-alb${NC}"
echo -e "${BLUE}   ECR: dev-bluepansy-api${NC}"
echo ""
