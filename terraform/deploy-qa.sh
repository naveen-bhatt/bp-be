...
# This script deploys the QA infrastructure using Terraform
...
ENV_DIR="$SCRIPT_DIR/environments/qa"

echo -e "${BLUE}🚀 QA Environment Deployment${NC}"
echo "=================================="
...
if ! aws ssm get-parameter --name "/qa/database_url" --region ap-south-1 &> /dev/null; then
    echo -e "${RED}❌ Parameter Store parameters not found. Please run the setup script first.${NC}"
    echo -e "${YELLOW}💡 Run: ./scripts/setup-parameter-store.sh qa${NC}"
    exit 1
fi
...
cd "$SCRIPT_DIR"
if ! ./validate-infrastructure.sh qa; then
    echo -e "${RED}❌ Infrastructure validation failed${NC}"
    exit 1
fi

echo -e "${GREEN}🎉 QA Environment deployment completed successfully!${NC}"
echo ""
echo -e "${BLUE}📋 Deployment Summary:${NC}"
echo -e "${BLUE}   Environment: QA${NC}"
echo -e "${BLUE}   ECS Cluster: qa-bluepansy-cluster${NC}"
echo -e "${BLUE}   ECS Service: qa-bluepansy-service${NC}"
echo -e "${BLUE}   ALB: qa-bluepansy-alb${NC}"
echo -e "${BLUE}   ECR: qa-bluepansy-api${NC}"
echo ""
